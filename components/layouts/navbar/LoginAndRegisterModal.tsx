...
const LoginAndRegisterModal = () => {
  const { isRegisterModal, isLoginOrRegister } = useSelector(
  ...
  return (
    <Modal
      ...
      onClose={() => dispatch(clientAction.handleShowRegisterModal(false))}
    >
      <ModalOverlay />
      <ModalContent>
        <ModalCloseButton />
        ...
        >
          ...
          >
            <Flex
              bg={isLoginOrRegister ? "#043bfe" : "transparent"}
              color={isLoginOrRegister ? "#fff" : "#6e7380"}
              onClick={handleChangeLoginOrRegister}
              sx={btnStyle}
            >
              {t("Login")}
            </Flex>
